...
def tox_add_option(parser: ToxParser):
    ...
    our.add_argument(
        "-d", action="store_true", help="list just default envs", dest="list_default_only"
    )
...
def list_env(state: State):
    ...
    extra = (
        [] if option.list_default_only else [e for e in state.tox_envs.keys() if e not in ignore]
    )
