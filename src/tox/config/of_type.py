...
class ConfigConstantDefinition(ConfigDefinition[T]):  # noqa: PLW1641
    ...
    def __call__(
        ...
        return cast("T", self.value() if callable(self.value) else self.value)  # ty: ignore[call-top-callable]
    ...
    def __call__(
        ...
        if self._cache is _PLACE_HOLDER:
            ...
            for loader in loaders:
                ...
                value = cast("T", self.default(conf, args.env_name) if callable(self.default) else self.default)  # ty: ignore[call-top-callable]
