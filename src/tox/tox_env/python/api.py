...
from tox.interpreters.discovery import PythonSpec, get_interpreter
...
class Python(ToxEnv, ABC):
    ...
    def default_base_python(self) -> List[str]:
        ...
        if spec.path is None:
            # if some version spec matched use it
            result.append(name)
        else:
            # otherwise just default to current executable
    ...
    def env_site_package_dir(self):
        ...
        site_at = next(
            Path(p) for p in reversed(python.path) if p.startswith(python.prefix)
        ).relative_to(Path(python.prefix))
    ...
    def install_python_packages(
        self, packages: List[Union[Path, Requirement]], no_deps: bool = False
    ) -> None:
        raise NotImplementedError
